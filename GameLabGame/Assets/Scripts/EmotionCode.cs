...
public class EmotionCode : MonoBehaviour
{
            ...
            if (link.GetLinkID() == "rainbow") {
                ...
                for (int i = link.linkTextfirstCharacterIndex; i < link.linkTextfirstCharacterIndex + link.linkTextLength; i++)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        Vector3 offset = new Vector2(0f, Mathf.Cos((Time.realtimeSinceStartup * movementSpeed) + (charInfo.vertexIndex * movementStrength.y))) * 10f;
